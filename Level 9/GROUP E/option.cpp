/*
included a function for call and put prices that return a Vector type of prices
that is defined in "UtilitiesDJD..."
*/
#include "option.hpp"
...
// overloaded functions with Vector from UtilitiesDJD
Vector<double, long> option::callPrice_withExcel(double S_start, double S_end, double diff)
{
	std::vector<double> S =meshArray(S_start, S_end, diff);
	Vector<double, long> price(S.size(),0.0); //create Vector that is the same size of S, and start the index at 0.
	for (int i = 0; i < S.size(); i++) {
		price[i] = call_Price(S[i], o_K, o_T, o_r, o_sig); //assign prices to price vector
	}
	return price;
}
Vector<double, long> option::putPrice_withExcel(double S_start, double S_end, double diff)
{
	std::vector<double> S = meshArray(S_start, S_end, diff);
	Vector<double, long> price(S.size(), 0.0);
	for (int i = 0; i < S.size(); i++)
	{
		price[i] = put_Price(S[i], o_K, o_T, o_r, o_sig);
	}
	return price;
}
