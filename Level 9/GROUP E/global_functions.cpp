/*
included a meshArray function that returns a Vector type defined in 
*/
...
{
	std::vector<double> model = meshArray(start, end, h);
	Vector<double, long> v(model.size(), 0.0);
	for (int i = 0; i < model.size(); i++)
	{
		v[i] = model[i]; //copy vector meshArray into Vector mesharray.
	}
	return v;
}
