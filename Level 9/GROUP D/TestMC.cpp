...
int main()
{
	...
	std::vector<double> priceVector; //create a vector to store prices to pass to error functions
	OptionData myOption;
	////BATCH 1 PUT
	//myOption.K = 65.0;
	//myOption.T = 0.25;
	...
	double tempPrice = 0.0; //another price variable to use for storing in price vector
	...
	{ // Calculate a path at each iteration
		...
		tempPrice += tmp;   //keep a runnning total of the prices, so we can average them for each i and put 
							//them in the price vector

		priceVector.push_back( (tempPrice / double (i)) * exp(-myOption.r * myOption.T) ); //add the price to the priceVector
		if ((i/10000) * 10000 == i)
		{// Give status after each 1000th iteration

				std::cout << i << std::endl;
				/*myfile << "simulation number: " << i << ".\n";
				myfile << "price = " << price * exp(-myOption.r * myOption.T) << ".\n"; */
		}
	...
	std::cout << "K = " << myOption.K << ".\n";
	std::cout << "T = " << myOption.T << ".\n";
	std::cout << "r = " << myOption.r << ".\n";
	std::cout << "sig = " << myOption.sig << ".\n";
	std::cout << "type = " << myOption.type << ".\n";
	std::cout << "S = " << S_0 << ".\n";
	std::cout << "standard deviation is: " << standardDev(priceVector, myOption.r, myOption.T) << endl;
	std::cout << "error is: " << standardErr(priceVector, myOption.r, myOption.T) << endl;
	/*myfile.close();*/
	return 0;
}